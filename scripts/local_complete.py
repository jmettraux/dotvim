...
fname_messages = '.local.messages.json'
...
if suc:
  content = cho['content']
  reasoning = cho['reasoning_content']
  if reasoning:
    print()
    print('  <!-- reasoning -->')
    print()
    #print(re.sub(r'\s+$', '', reasoning, flags=re.MULTILINE))
    print(reasoning)
    print('  <!-- / reasoning -->')
    print()
  #print(re.sub(r'\s+$', '', content, flags=re.MULTILINE))
  print(content)
else:
  print('```')
